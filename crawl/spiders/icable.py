...
def get_epgs_icable(channel, channel_id, dt, func_arg):
    ...
    msg = ""
    success = 1
    url = "http://epg.i-cable.com/ci/channel/epg/%s/%s?api=api&locale=chi" % (
        channel_id,
        dt.strftime("%Y%m%d"),
    )
    try:
...
def get_channels_icable():
    ...
    for c in channels_json:
        ...
        logo = (
            "http://epg.i-cable.com/new/images/epgMobileIcon/%s.png" % c["channel_no"]
        )
