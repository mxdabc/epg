...
class Channel(models.Model):
    ...
    tvg_name = models.CharField(
        "匹配用的tvg-name", null=False, max_length=100, db_index=True
    )
    name = models.CharField("频道显示名称", null=False, max_length=100, db_index=True)
    sort = models.CharField(
        "频道分类", null=False, max_length=50, db_index=True
    )  # 1，2，3，4，5
    ...
    has_epg = models.IntegerField(
        "是否有节目表", choices=need_get, default=0, db_index=True
    )
    ineed = models.IntegerField(
        "是否需获取", choices=need_get, default=0, db_index=True
    )
    source = models.CharField(
        "节目来源", choices=source_choices, max_length=50, db_index=True, null=True
    )
    recrawl = models.IntegerField(
        "需重新获取当天数据", choices=need_get, db_index=True, default=0
    )
    ...
    def __str__(self):
        return "-".join(
            [str(self.id), self.name, self.tvg_name, self.source if self.source else ""]
        )
    ...
    def get_crawl_channels(self, need_date, recrawl=0):
        if recrawl == 1:
            return self.objects.filter(ineed=1, has_epg=1).filter(
                Q(recrawl=1) | Q(last_program_date__lt=need_date)
            )
        else:
            return self.objects.filter(
                has_epg=1, ineed=1, last_program_date__lt=need_date
            )
...
class Epg(models.Model):
    ...
    def __str__(self):
        return "%s %s %s" % (
            self.channel_id,
            self.starttime.astimezone(tz=tz_sh),
            self.title,
        )
    ...
    def get_epgs(self, channel_ids, need_program_date):
        if need_program_date > datetime.datetime.now().date():
            epgs = self.objects.filter(
                ...
                program_date__gte=datetime.datetime.now(),
                program_date__lte=need_program_date,
            )  # ,program_date__lte=program_date,program_date__gte=datetime.datetime.now().date())
            if epgs.count() == 0:  # 没有数据则获取当天数据
                epgs = self.objects.filter(
                    channel_id__in=channel_ids,
                    program_date=datetime.datetime.now().date(),
                )
        else:
            epgs = self.objects.filter(
                channel_id__in=channel_ids, program_date=need_program_date
            )
        for epg in epgs:
            if epg.endtime is None:
                epg.endtime = datetime.datetime.combine(
                    epg.starttime.date(), datetime.time(hour=23, minute=59, second=59)
                )
        return epgs
    ...
    def get_single_epg(self, channel, need_date):
        epgs_list = []
        no_endtime_endtime = datetime.datetime.combine(
            need_date, datetime.time(hour=12, minute=59, second=59)
        ).astimezone(tz=tz_sh)
        ...
        for (
            ...
            epgs
        ):  # 将结束时间为空白的字段，加上当天最晚一秒.直接在此处处理时区问题，后面获取后不再处理
            if epg.endtime is None:
    ...
    def save_to_dbs(self, ret):
        ...
        if ret["source"] in [
            ...
            cs = self.objects.filter(
                channel_id=ret["epgs"][0]["channel_id"],
                starttime__lt=ret["epgs"][0]["starttime"].astimezone(tz=tz_sh),
            )
            if cs:
                cs = cs.latest("starttime")
                if (
                    ret["epgs"][0]["starttime"].astimezone(tz=tz_sh)
                    - cs.starttime.astimezone(tz=tz_sh)
                ).seconds < 19000:  # 只有库里最新的记录，离当前第一条记录小于2.5小时才会填充前一条记录。
        ...
        for epg in ret["epgs"]:
            try:
                ...
                querye = Epg(
                    ...
                    starttime=epg["starttime"].astimezone(tz=tz_sh),
                    endtime=(
                        epg["endtime"].astimezone(tz=tz_sh) if epg["endtime"] else None
                    ),
                    title=epg["title"],
                    descr=descr,
                    program_date=epg["program_date"],
                    source=ret["source"],
                )
...
class Channel_list(models.Model):
    ...
    is_alive = models.IntegerField(
        "来源网站上是否仍然存在", choices=is_alive_choice, default=1
    )
